...
const BLP_CONTENT_JPEG = 0;
const BLP_CONTENT_DIRECT = 1;
const DEFAULT_MIP_LEVEL = 0;
...
function createImageDataLike(width: number, height: number): ImageDataLike {
	return {
		width,
		height,
...
function decodeBlp(bytes: Uint8Array): BlpImage {
	const data = toTightArrayBuffer(bytes);
	const view = new DataView(data);
	const type = keyword(view, 0);

	if (type === 'BLP0' || type === 'BLP2') {
		throw new Error(type === 'BLP2' ? localize('blp2.unsupportedPreview') : 'BLP0 preview is not supported');
	}
	if (type !== 'BLP1') {
	...
	const image: BlpImage = {
		content: uint32(view, 1),
		alphaBits: uint32(view, 2),
		width: uint32(view, 3),
		height: uint32(view, 4),
		mipmaps: [],
		data
	};

	if (image.content !== BLP_CONTENT_JPEG && image.content !== BLP_CONTENT_DIRECT) {
		throw new Error(localize('blp1.unsupportedCompression', { compression: image.content }));
	}
...
function getBlpImageData(blp: BlpImage, mipmapLevel: number): ImageDataLike {
	const view = new DataView(blp.data);
	const uint8Data = new Uint8Array(blp.data);
	const mipmap = blp.mipmaps[mipmapLevel];

	if (!mipmap) {
		throw new Error(localize('blp1.jpegEmptyMipmap'));
	}

	if (blp.content === BLP_CONTENT_JPEG) {
		const headerSize = uint32(view, 39);
		const data = new Uint8Array(headerSize + mipmap.size);
	...
	const width = blp.width / (1 << mipmapLevel);
	const height = blp.height / (1 << mipmapLevel);
	...
	const imageData = createImageDataLike(width, height);
...
export function decodeBlpForPreview(bytes: Uint8Array): BlpRgbaImage {
	const blp = decodeBlp(bytes);
	const imageData = getBlpImageData(blp, DEFAULT_MIP_LEVEL);
